...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        "--data-path",
        default="/data/cmpe249-fa23/WaymoCOCO/",
        type=str,
        help="dataset path",
    )  # "/data/cmpe249-fa23/WaymoCOCO/"
    parser.add_argument(
        "--annotationfile",
        default="",
        type=str,
        help="dataset annotion file path, e.g., coco json file",
    )  # annotations_train200new.json
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        "--model", default="customrcnn_resnet152", type=str, help="model name"
    )  # customrcnn_resnet152, fasterrcnn_resnet50_fpn_v2
    parser.add_argument(
        "--trainable",
        default=0,
        type=int,
        help="number of trainable layers (sequence) of backbone",
    )
    parser.add_argument(
        "--device",
        default="cpu",
    ...
    )  # /data/cmpe249-fa23/trainoutput/kitti/model_4.pth
    ...
    parser.add_argument(
        "--multigpu", default=False, type=bool, help="disable torch ddp"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--expname", default="0315", help="experiment name, create a sub-folder"
    )
...
def main(args):
   ...
   # Data loading code
    ...
    train_subset_size = int(len(dataset) * 0.1)
    train_indices = torch.randperm(len(dataset))[:train_subset_size].tolist()
    dataset = torch.utils.data.Subset(dataset, train_indices)

    # Create a subset with 10% of the test data
    test_subset_size = int(len(dataset_test) * 0.1)
    test_indices = torch.randperm(len(dataset_test))[:test_subset_size].tolist()
    dataset_test = torch.utils.data.Subset(dataset_test, test_indices)

    print("Creating data loaders")
    print(f"Train set len: {len(dataset)} (10% of original {len(train_indices)})")
    print(f"Test set len: {len(dataset_test)} (10% of original {len(test_indices)})")
    ...
    model, preprocess, classes = create_detectionmodel(
        args.model, num_classes, args.trainable
    )
    # model.to(device)
    ...
    if args.test_only:
        ...
        simplemodelevaluate(model, data_loader_test, device="cpu")
...
def train_one_epoch(
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.amp.autocast(device_type='cpu', enabled=scaler is not None):
